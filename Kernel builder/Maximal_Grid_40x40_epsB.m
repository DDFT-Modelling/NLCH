...
%% 40 x 40 base grid
% Factor 1   takes 8 min 53.45 s
% Factor 2.5 takes 38 min 6.86 s
% Factor 4   takes 1 h 42 min 27 s

% The following takes around 2 h 28 min
k = 1;
T = zeros(1,3);
Factors = [1,2.5,4];
Range_Epsilons = [1e-5];
for eps = Range_Epsilons 
    ...
    for i = Factors
        i
        tic;
        [aBox,MS, errors, t_Full, Conv, experiment, smaller, full_thing, all_sq] = SingularKernelIntergralMS_Conv_Maximal_Q(eps, 40, 40, i);
        T(k)= toc;  % pair 1: toc
...
xticks([1, 1.5, 2.0])
% 
divisions = {'40/1','40/2.5','40/4'};
xlabelArray = [ divisions; {' ',' 10^{-5}',' '} ]; 
...
Newtonians.Level.n2_5 = Conv_Out_40{2};
Newtonians.Level.n4   = Conv_Out_40{3};

save('Singular_Kernels_Subs_40_epsB.mat', 'Newtonians');
