function Rho_t = NL_CH_Integrator_Source_DAE(phi_ic,  aBox, Diff, Ind,  Conv, Conv_D, TimeLine)
    ...
    y1 = aBox.Pts.y1_kv;
    y2 = aBox.Pts.y2_kv;
    phi_0 = sin( 2.0 * y1 * pi ) .* cos( 2.0 * y2 * pi );   % Underlying base from exact solution
    ...
    Tmax = 1; %1e-2; %2e-1; %1e-1; % 5e-2; %10.0; %TimeLine.Pts.y(2);
    Tmin = 0.0; %1e-1;%5e-2; %1e-15;
    h    = (Tmax-Tmin)/99;
    ...
    phi = ( (Time_Interval>0).*exp(-Time_Interval) + (Time_Interval==0)*exp(-1e-15) )' * phi_0';
    sigma = dF(phi) - (Conv * phi' + Conv_D .* phi')';
    dsigma = (normal * grad * sigma');

    % 
    d_bound = A_bound_inv * ( dsigma + B_No_Flux(:,~bound) * Phi_t' - normal_grad(:,~bound) * dF(Phi_t')  );
    % Find a fixed point
    ...
    function dydt = rhs(t,rho_inner)
        %t
        rho_inner = clip(rho_inner,-1+1e-15,1+1e-15);

        %% Compute source terms
        % Compute σ at time t
        phi    = ( (t>0)*exp(-t) + (t==0)*exp(-1e-15) ) * phi_0;  % σ should not be evaluated at time 0 but we prevent this
        sigma  = dF(phi) - (Conv * phi + Conv_D .* phi);
        dsigma = normal * grad * sigma;
        % Compute g at time t
        g = -phi - (Lap_Space * sigma);


        %% Interior-to-boundary
        % Compute constant vector
        d_bound = A_bound_inv * ( dsigma + B_No_Flux(:,~bound) * rho_inner - ...
