...
% CASE B
phi_ic = -0.5 * ones(N1*N2,1);


%% Solve equation iteratively
Ens  = struct('A',[], 'B',[], 'C',[], 'D',[], 'E',[], 'F',[]);
etas = [300 100  1  -15  -100  -150];
...
for i = 1:numel(Fields)
    ...
    Out_Name = strcat(['B_eta=',int2str(eta), '.gif']);
    ...
    Out_Name = strcat(['B',int2str(i), '_Solution_eta=',int2str(eta), '.pdf']);
    plot_panels
end
...
exportgraphics(figure(1), 'NLCH_Energy_B.pdf', 'BackgroundColor','none', 'ContentType', 'vector', 'Resolution', 300)
hold off
