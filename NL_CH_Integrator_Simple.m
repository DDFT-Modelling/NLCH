function Phi_t = NL_CH_Integrator_Simple(phi_ic,  aBox, Diff, Ind,  Conv, Conv_D, dF, ddF, TimeLine, eta, Tmin )
...
if nargin <= 10
  Tmin = 0.0;
    ...
    Tmax = 1;
    %Tmax = 1e-3;
    %Tmax = 10.0 - 1e-3;% - (2/3) * 1e-3; %1.0/375; %(2/3) * 1e-3; %1.0/375; %(1/3) * 1e-2; % %10.0; %TimeLine.yMax;
    %Tmax = 10.0;
    h    = (Tmax - Tmin)/99;
    ...
    function f = getFlux(phi)
        ...
        f1 = 1 * dF(z) - eta * (Conv * phi + Conv_D .* phi);
        f = grad * f1;
    end
