classdef MultiShape < handle
    ...
    properties (Access = public)
        ...
        % whether the multishape is periodic in each dimension
        periodic = [false;false]
        periodicL = [NaN;NaN] % periodic length
    ...
    methods
        ...
        function this = MultiShape(shapes,normalsOverride,opts)
            ...
            fields = {'doDiff','doInd','doInt','doInterp'};
            for iField = 1:length(fields)
                if(~isfield(opts,fields{iField}))
                    opts.(fields{iField}) = true;
                end
            end
            ...
            % determine if the shapes are periodic
            % *** Assumes they are all the same as the first shape ***
            if(this.Shapes(1).Shape.periodic(1))
                this.periodic(1) = true;
                this.periodicL(1) = this.Shapes(iShape).Shape.L1;
            end
            if(this.Shapes(1).Shape.periodic(2))
                this.periodic(2) = true;
                this.periodicL(2) = this.Shapes(iShape).Shape.L2;
            end
        ...
        % *** Note that all points in MS are in Cartesian coordinates ***
            ...
            % basis
            this.GetShapeMasks();
            this.Pts.y1_Original = this.Pts.y1_kv;
            this.Pts.y2_Original = this.Pts.y2_kv;
        ...
        function ComputeInterpolationMatrices(this,interpGrid1,interpGrid2)
            
            % default grid
            if(nargin==1)
                if(this.periodic(1))
                    interpGrid1 = (0:0.01:1)';
                else
                    interpGrid1 = (-1:0.01:1)';
                end
                if(this.periodic(2))
                    interpGrid2 = (0:0.01:1)';
                else
                    interpGrid2 = (-1:0.01:1)';
                end                
            elseif(nargin==2)
                interpGrid2 = interpGrid1;
            end
            ...
            for iShape = 1:this.nShapes
                ...
                IPi = this.Shapes(iShape).Shape.ComputeInterpolationMatrix(interpGrid1,interpGrid2,true,true);
...
%             Interp.pts2 = Pts.y2_kv;
        ...
        function ComputeIndices(this,normalsOverride)
            ...
            for iDup = 1:size(duplicates,1)
                ...
                if(isempty(normalTemp))
                    ...
                    normalBoundIdx = zeros(nMatches,2*this.M);
                    % extract normal vectors and sum
                    for iMatch = 1:nMatches
                        
                        % finding indices in normal matrix corresponding to
                        % matches
                        boundIdx = zeros(size(boundPts,1),1);
                        boundIdx(matches(iMatch)) = 1;
                        boundIdxFull = zeros(this.M,1);
                        boundIdxFull(this.Ind.bound) = boundIdx;
                        normalBoundIdx(iMatch,:) = this.MakeVector(boundIdxFull,boundIdxFull);
                        normalMatch = normal(matches(iMatch),:);
                        
                        %normalMatch(normalMatch==0) = []; % don't delete this yet
                        normalMatch(~normalBoundIdx(iMatch,:)) = [];
                        % checking that variable has right size
                        if length(normalMatch) < 2
                            return
                            disp('only one normal found')
                        end
                 ...
                 for iMatch = 1:nMatches
                    ...
                    %normalMatch(normalMatch~=0) = normalTemp; % don't delete this yet

                    normalMatch(normalBoundIdx(iMatch,:)~=0) = normalTemp;
        ...
        function M_conv = ComputeConvolutionMatrix(this,f,saveBool)
            ...
            for i=1:this.M
                if(useDistance)
                    fP          = f(this.GetDistance(y1(i) - y1,y2(i) - y2));
                else
                    % deal with possible periodicity
                    [~,dy1,dy2] = this.GetDistance(y1(i) - y1,y2(i) - y2);
                    fP          = f(dy1,dy2);
                end
                Mmask{1} = i;
        ...
        function [d,dy1,dy2] = GetDistance(this,dy1,dy2)
            % inputs should be the separation of points in y1 and y2
            
            % deal with possible periodicity
            if(this.periodic(1))
                dy1 = this.GetDistancePeriodic(dy1,1);
            end
            if(this.periodic(2))
                dy2 = this.GetDistancePeriodic(dy2,2);
            end

            d = sqrt( dy1.^2 + dy2.^2 );
        end

        function d  = GetDistancePeriodic(this,d,dim)
            % in 1D for a periodic dimension
            % assumes that the domain is [0,L]
            L = this.periodicL(dim);
            mask = ( abs(d) > L/2 );
            d(mask) = d(mask) - sign(d(mask))*L;
        end
        ...
        function dprod = dotProduct(this, u, v)
            ...
            dprod = zeros(length(this.Pts.y1_kv), 1);
            for iShape = 1:this.nShapes              
                mask = this.Shapes(iShape).PtsMask;
                u1p = u1(mask);
                u2p = u2(mask);
                ...
                v2p = v2(mask);
                if(strcmp(this.Shapes(iShape).Shape.polar,'polar'))
                    dprod(mask) = (u1p.*v1p).*cos(u2p - v2p);
        ...
        % copy for upper/lower case convention
        ...
        function SetAxes(this,ha)
            ...
            % deal with shapes such as InfCapillary
            if(~isfinite(xMin))
                y1 = this.Pts.y1_kv;
                xMin = min(y1(isfinite(y1)));
            end
            if(~isfinite(xMax))
                y1 = this.Pts.y1_kv;
                xMax = max(y1(isfinite(y1)));
            end
            if(~isfinite(yMin))
                y2 = this.Pts.y2_kv;
                yMin = min(y2(isfinite(y2)));
            end
            if(~isfinite(yMax))
                y2 = this.Pts.y2_kv;
                yMax = max(y2(isfinite(y2)));
            end
            
            % deal with periodic domains
            if(this.periodic(1))
                xMax = xMin + this.periodicL(1);
            end
            if(this.periodic(2))
                yMax = yMin + this.periodicL(2);
            end
        ...
        %------------------------------------------------------------------
        % Matching conditions for optimal control problems
        %------------------------------------------------------------------

        function dydt = ApplyIntersectionFlux(this,dydt,data,BCs)
            % loop through pairs of shapes
            for iShape = 1:this.nShapes
                for jShape = iShape+1:this.nShapes
                    ...
                    % if they intersect apply the given BC function
                    if(~isempty(BCs(iShape,jShape).function))                        
                        BCF = BCs(iShape,jShape).function;
                        BCF2= strcat(BCF,"Flux");
                        BCfn = str2func(BCF2);
                        [BC,mask] = BCfn(data,this.Intersections,iShape,jShape);
                        dydt(mask,:) = BC(mask,:);
                    end
        ...
        function dydt = ApplyIntersectionRho(this,dydt,data,BCs)
            % loop through pairs of shapes
            for iShape = 1:this.nShapes
                for jShape = iShape+1:this.nShapes
