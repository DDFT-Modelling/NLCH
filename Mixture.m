...
ConvL = Conv + (0.025) * H_a;  % Mixed kernel


%% Solve equation iteratively
Ens  = struct('A',[], 'B',[], 'C',[], 'D',[], 'E',[], 'F',[],'G',[]);
etas = [1000, 500, 300, 100, -100, -300, -500];
...
for i = 1:numel(Fields)
    eta = etas(i);
    % Solve equation
    %Phi_to = NL_CH_Integrator_DAE( phi_ic,  aBox, Diff, Ind,  ConvL, Conv_D, eta, TimeLine);
    % If process gets stuck (-150), then try:
    Phi_to = NL_CH_Integrator_Simple(phi_ic,  aBox, Diff, Ind,  ConvL, Conv_D, dF, ddF, TimeLine, eta, 0.0 );
    % Energy
    Energy = (Int * F(Phi_to)') - 0.5 * eta * Int * ( (ConvL * Phi_to' + Conv_D .* Phi_to') .* Phi_to');
    Ens.(Fields{i}) = Energy;

    % Plot solution as an animation
    Out_Name = strcat(['E_eta=',int2str(eta), '.gif']);
    plots_in_box(Phi_to, aBox, outTimes, Out_Name, '$\rho(x;t)$')

    % Equilibria
    C_Val_Approx = dF(Phi_to(end,:)') - eta * (ConvL * Phi_to(end,:)' + Conv_D .* Phi_to(end,:)');
    %aBox.plot(C_Val_Approx);
    max(C_Val_Approx) - (max(C_Val_Approx) + min(C_Val_Approx))/2

    % Modify plot_panels for this:
    Out_Name = strcat(['E',int2str(i), '_Solution_eta=',int2str(eta), '.pdf']);
    plot_panels
end
...
exportgraphics(figure(1), 'NLCH_Energy_E.pdf', 'BackgroundColor','none', 'ContentType', 'vector', 'Resolution', 300)
hold off
